...
export const UserList = ({ setTeam, team }) => {
  ...
  return (
    <div>
      <p className="text-gray-700">Asignar tarea: </p>
      <Listbox
        value={selectedUsers}
          ...
          >
            <ListboxOptions className="z-50 absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
              {data?.map((user, index) => (
                <ListboxOption
                  key={index}
                  className="relative cursor-default select-none py-2 pl-10 pr-4 data-[active]:bg-amber-100 data-[active]:text-amber-900 data-[inactive]:text-gray-900"
                  value={user}
                >
                  {({ selected }) => (
                    <>
