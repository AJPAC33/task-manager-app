...
export const createTask = async (req, res) => {
  try {
    ...
    let text = "Una nueva tarea se te ha asignado a ti";
    if (team?.length > 1) {
      text = text + ` y otros más.`;
    }

    text =
      text +
      ` La prioridad de la tarea ha sido cambiada a ${priority} prioridad, revisa y actua de acuerdo a ello. La fecha de la tareas es ${new Date(
        date
...
export const duplicateTask = async (req, res) => {
  try {
    ...
    text =
      text +
      ` La prioridad de la tarea ha sido cambiada a ${
        task.priority
      } prioridad, revisa y actua de acuerdo a ello. La fecha de la tareas es ${task.date.toDateString()}. Gracias !!!`;
...
export const dashboardStatistics = async (req, res) => {
  try {
    ...
    stages.forEach((stage) => {
      if (!groupTasks[stage]) groupTasks[stage] = 0;
    });
