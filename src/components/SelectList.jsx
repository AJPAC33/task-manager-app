...
export const SelectList = ({ options, selected, setSelected, label }) => {
  return (
          ...
          >
            <ListboxOptions className="z-50 absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
              {options.map((option, index) => (
                <ListboxOption
                  key={index}
                  className="relative cursor-default select-none py-2 pl-10 pr-4data-[active]:bg-amber-100 data-[active]:text-amber-900 data-[inactive]:text-gray-900"
                  value={option.value}
                >
                  {({ selected }) => (
                    <>
