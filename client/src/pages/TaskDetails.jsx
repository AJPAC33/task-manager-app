...
const Activities = ({ activity, id, refetch }) => {
  const [selected, setSelected] = useState(ACT_TYPES_MAP[act_types[0]]);
        ...
        <div className="w-full">
          {activity?.map((el, index) => (
            <Card
              ...
              item={{
                ...el,
                label: ACT_TYPES_LABELS[el.type], // 👈 aquí ya se está generando en español
              }}
          ...
          {act_types.map((item) => (
            <div key={item} className="flex gap-2 items-center">
              <input
                type="checkbox"
                className="w-4 h-4"
                checked={selected === ACT_TYPES_MAP[item] ? true : false}
                onChange={(e) => setSelected(ACT_TYPES_MAP[item])}
              />
              <p>{item}</p>
            </div>
          ))}
